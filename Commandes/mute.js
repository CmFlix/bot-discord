...
module.exports = {
    ...
    async run(bot, message, args) {
        ...
        if(ms(time) > 2419200000)  return message.reply("le mute ne  doit pas durée plus de 28 jours !")
        ...
        try {await user.send(`Vous avez été mute du serveur ${message.guild.name} par ${message.user.tag} pendant ${time} pour la raison: \`${reason}\``)} catch(err) {}
